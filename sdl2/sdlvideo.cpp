...
void S9xInitDisplay (int argc, char **argv)
{
	 ...
	 */	
        ...
        {
			float fill = 1.075f;
                  GUI.sdl_window =  SDL_CreateWindow("SDL2 Example", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, sWith, sHeight, 0);
               GUI.sdl_renderer = SDL_CreateRenderer(GUI.sdl_window, 0, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
			    GUI.sdl_screen = SDL_CreateRGBSurfaceWithFormat(0, SNES_WIDTH * 2, SNES_HEIGHT_EXTENDED * 2, 16, SDL_PIXELFORMAT_RGB565);
				float modsize = (sHeight/(SNES_HEIGHT_EXTENDED ));
				std::cout << "test"<<modsize<<std::endl;
				sdl_sizescreen.w =((SNES_WIDTH )* modsize)*fill  ;
				sdl_sizescreen.h = ((SNES_HEIGHT_EXTENDED)* modsize) * fill;
				sdl_sizescreen.y = 0;
				sdl_sizescreen.x = (sHeight - (SNES_WIDTH )) /2;
        } else {
               GUI.sdl_window =  SDL_CreateWindow("SDL2 Example", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,SNES_WIDTH * 2, SNES_HEIGHT * 2, 0);
               GUI.sdl_renderer = SDL_CreateRenderer(GUI.sdl_window, 0, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
			   GUI.sdl_screen = SDL_CreateRGBSurfaceWithFormat(0, SNES_WIDTH * 2, SNES_HEIGHT_EXTENDED * 2, 16, SDL_PIXELFORMAT_RGB565);
